# LVS (Linux Virtual Server) 負載均衡 PoC — 完整教學

> 本教學帶你從零搭建一套 **Layer-4 負載均衡叢集**，採用 LVS / IPVS 的 **DSR (Direct Server Return)** 模式，並以 **Keepalived + VRRP** 實現 Director 高可用。適合初學者理解「真正的生產級負載均衡」如何運作。
...
2. [核心概念速覽](#2-核心概念速覽)
3. [整體架構](#3-整體架構)
4. [三種 LVS 轉發模式比較](#4-三種-lvs-轉發模式比較)
...
## 1. 為什麼需要 Load Balancer？

當單一伺服器無法承受所有流量時，我們需要**多台伺服器**分擔工作。Load Balancer（負載均衡器）就是站在「前面」的交通警察：

```
                    沒有 LB                          有 LB
                  ┌─────────┐              ┌─────────────────┐
  全部流量 ──────►│ 單一伺服器│    流量 ────►│  Load Balancer   │
                  │ (容易掛) │              └───┬──────┬──────┘
                  └─────────┘                  │      │      │
...
| 好處 | 說明 |
|------|------|
| **可擴展性 (Scalability)** | 流量變大？加機器就好 |
| **高可用 (High Availability)** | 一台掛了自動切換到其他台 |
| **靈活部署** | 可以做滾動升級、A/B 測試 |
...
| 術語 | 全名 | 白話解釋 |
|------|------|---------|
| **LVS** | Linux Virtual Server | Linux 內建的 Layer-4 負載均衡框架 |
| **IPVS** | IP Virtual Server | LVS 在 kernel 裡的實作模組 |
| **VIP** | Virtual IP | 對外公開的 IP，Client 只認這個 |
| **Director** | — | 負責分配流量的節點（運行 IPVS） |
| **Real Server (RS)** | — | 實際處理請求的後端伺服器 |
| **DSR / DR** | Direct Server Return / Direct Routing | 回應不經過 Director，直接回 Client |
| **VRRP** | Virtual Router Redundancy Protocol | 讓多個 Director 共享 VIP 的協定 |
| **Keepalived** | — | 實作 VRRP + 健康檢查的 daemon |
| **WRR** | Weighted Round Robin | 按權重輪流分配請求 |
...
```
                            ...
                            └──────────┬──────────┘
                                       │
                            ① Client 送 Request 到 VIP
                                 192.168.100.100
                                       │
                    ┌──────────────────┴──────────────────┐
                    │          VRRP (Keepalived)           │
                    │    ② VIP 浮動在 MASTER Director 上   │
                    │                                      │
              ┌─────┴─────┐                         ┌─────┴─────┐
...
1. **Client → VIP**：Client 送 HTTP request 到 VIP `192.168.100.100`
2. **VIP → Director**：因為 VIP 綁在 MASTER Director 上，Director 收到封包
3. **Director → Real Server**：IPVS 透過 DR 模式轉發（只改 MAC，不改 IP）
4. **Real Server → Client**：RS 直接回應 Client（**不經過 Director**）
...
```
 ╔═══════════════╦═════════════════╦═════════════════╦══════════════════╗
 ║               ║   NAT 模式       ║   DR 模式 (DSR)  ║   TUN 模式       ║
 ╠═══════════════╬═════════════════╬═════════════════╬══════════════════╣
 ║ Request 路徑   ║ Client→Dir→RS   ║ Client→Dir→RS   ║ Client→Dir→RS   ║
 ║ Response 路徑  ║ RS→Dir→Client   ║ RS→Client ⭐    ║ RS→Client ⭐    ║
 ║ Director 負擔  ║ 高 (雙向流量)    ║ 低 (只看 Request) ║ 低 (只看 Request) ║
 ║ RS 需要改設定  ║ 只改 Gateway     ║ ARP + lo VIP     ║ IP Tunnel        ║
 ║ 跨子網         ║ ✅              ║ ❌ 同一子網       ║ ✅               ║
 ║ 適合場景       ║ 小規模          ║ 大規模 Web ⭐    ║ 跨機房           ║
 ╚═══════════════╩═════════════════╩═════════════════╩══════════════════╝
```

**本 PoC 選擇 DR (DSR) 模式**，因為：
- Response 通常比 Request 大很多（想想下載一張圖片：Request 100 bytes，Response 5MB）
- Director 只處理小小的 Request，throughput 可以非常高
- 適合大部分在同一機房的 Web 應用
...
DSR 是本 PoC 的核心，讓我們用封包層級來理解：

### 5.1 封包在每一站的變化

```
Step 1: Client → Director
┌──────────────┬──────────────┬────────────────┐
│ Src MAC:     │ Src IP:      │                │
│  Client MAC  │  Client IP   │  HTTP Request  │
│ Dst MAC:     │ Dst IP:      │                │
│  Dir MAC     │  VIP ⭐      │                │
└──────────────┴──────────────┴────────────────┘

Step 2: Director → Real Server (DR 轉發)
┌──────────────┬──────────────┬────────────────┐
│ Src MAC:     │ Src IP:      │                │
│  Dir MAC     │  Client IP   │  HTTP Request  │
│ Dst MAC:     │ Dst IP:      │  (不變)        │
│  RS MAC ⭐   │  VIP ⭐      │                │
└──────────────┴──────────────┴────────────────┘
   ▲                ▲
...
Step 3: Real Server → Client (直接回應)
┌──────────────┬──────────────┬────────────────┐
│ Src MAC:     │ Src IP:      │                │
│  RS MAC      │  VIP ⭐      │  HTTP Response │
│ Dst MAC:     │ Dst IP:      │  (5MB 圖片)   │
│  GW MAC      │  Client IP   │                │
└──────────────┴──────────────┴────────────────┘
   ▲
   RS 用 VIP 當 Src IP → Client 覺得是 VIP 回的
   完全不經過 Director！
```
...
**a) VIP 綁在 loopback 上**
```bash
# RS 需要「認識」VIP，否則收到目的地是 VIP 的封包會丟棄
ip addr add 192.168.100.100/32 dev lo label lo:0
```

**b) ARP 抑制**
```bash
# RS 不能回應「誰是 VIP？」的 ARP 查詢，否則會搶走 Director 的流量
net.ipv4.conf.all.arp_ignore = 1    # 只回應自己網卡上的 IP
net.ipv4.conf.all.arp_announce = 2  # ARP 公告時用最適合的 IP
```

---

## 6. VRRP 與 Keepalived 高可用

### 6.1 問題：Director 是單點故障

如果只有一個 Director，它掛了整個服務就斷了。解法是用 **VRRP 協定** 讓兩個 Director 共享 VIP。
...
└─────────────┘                  └─────────────┘
     ...
     VIP 在 Director-1              VIP 自動轉移到 Director-2
...
| 功能 | 說明 |
|------|------|
| **VRRP** | 管理 VIP 在 MASTER/BACKUP 間的漂移 |
| **Health Check** | 定期 HTTP GET `/health` 檢查 RS 健康 |
| **IPVS 管理** | 自動新增/移除 Real Server 到 IPVS 表 |

---

## 7. WRR 排程演算法

WRR (Weighted Round Robin) 按權重分配請求：

```
Real Server    Weight    預期流量佔比
─────────────────────────────────────
RS-1           3         60%  ████████████
RS-2           2         40%  ████████
```

如果有 100 個請求，大約 60 個給 RS-1，40 個給 RS-2。

**其他常見排程算法：**

| 算法 | 說明 | 適合場景 |
|------|------|---------|
| `rr` | Round Robin | 所有 RS 性能相同 |
| `wrr` | Weighted RR ⭐ | RS 性能不同 |
| `lc` | Least Connection | 長連接 |
| `wlc` | Weighted LC | 長連接 + 不同性能 |
| `sh` | Source Hash | 需要 Session 親和 |
...
- **Linux 核心**需支援 IPVS 模組 (`ip_vs`, `ip_vs_wrr`, `ip_vs_rr`)
...
| 面向 | 選擇 | 理由 |
|------|------|------|
| **LB 模式** | DR (DSR) | Response 繞過 Director → 高 throughput |
| **排程算法** | WRR | 支援異質後端 (不同性能) |
| **高可用** | Keepalived + VRRP | 業界標準，亞秒級 failover |
| **健康檢查** | HTTP GET /health | 應用層檢查，而非只是 TCP |
| **Session 親和** | 60s source-hash | 有狀態應用的 session affinity |
| **ARP 處理** | arp_ignore=1, arp_announce=2 | 防止 RS 搶 VIP 的 ARP |
...
| IPVS 核心模組 | Linux 內建 | `lsmod \| grep ip_vs` |
...
> **看到 `ip_vs` 就對了！** 這代表 Linux kernel 的 IPVS 子系統已經載入，Docker 容器可以直接使用。
...
docker/client/Dockerfile                         # Client 容器定義
docker/client/test-scripts/test-all.sh           # 完整測試套件
docker/client/test-scripts/test-basic.sh         # 基本連線測試
docker/client/test-scripts/test-distribution.sh  # 負載分配測試
docker/client/test-scripts/test-dsr-verify.sh    # DSR 封包驗證
docker/client/test-scripts/test-failover.sh      # Failover 監控
docker/client/test-scripts/test-performance.sh   # 效能壓力測試
docker/director/Dockerfile                       # Director 容器定義
docker/director/entrypoint.sh                    # Director 啟動腳本
docker/realserver/Dockerfile                     # Real Server 容器定義
docker/realserver/entrypoint.sh                  # Real Server 啟動腳本
```

```
專案架構：

docker-compose.yml          ← 定義 5 個容器和網路
manage.sh                   ← 叢集管理腳本 (up/down/test/failover...)
docker/
├── director/
│   ├── Dockerfile          ← Ubuntu 22.04 + ipvsadm + keepalived
│   └── entrypoint.sh       ← 動態生成 keepalived.conf，啟動 Keepalived
├── realserver/
│   ├── Dockerfile          ← Nginx 1.24 + iproute2
│   └── entrypoint.sh       ← ARP 抑制 + VIP on loopback + 啟動 Nginx
└── client/
    ├── Dockerfile          ← Ubuntu 22.04 + curl + ab + tcpdump
    └── test-scripts/       ← 5 個自動化測試腳本
```
...
```
🚀 Starting LVS PoC Cluster (Docker)...

Building and starting: Directors → Real Servers → Client
```

Docker 會自動 build 三種映像檔（首次約 1-2 分鐘），然後按順序啟動容器。

等待 10 秒讓 Keepalived 穩定後，自動顯示叢集狀態：

```
╔════════════════════════════════════════════════╗
║   LVS PoC Cluster Status                      ║
╚════════════════════════════════════════════════╝
...
這張表告訴你：
- **VIP**: `192.168.100.100:80` — 客戶端存取這個 IP
- **Scheduler**: `wrr` — 加權輪詢排程
- **Forward**: `Route` — 就是 DR (DSR) 模式
- **Weight**: `3` 和 `2` — RS-1 接收 60% 流量，RS-2 接收 40%

> **檢查重點 2：VIP 位置和 RS 健康狀態**

```
── VIP Location ──
  ✅ VIP is on: lvs-director-1

── Real Server Health ──
  ✅ real-server-1 (192.168.100.21): Healthy
  ✅ real-server-2 (192.168.100.22): Healthy
```
...
── Test 3: Server Identity (10 requests) ──
  Request 1: Served by real-server-1     ← weight=3，出現較多
  Request 2: Served by real-server-1
  Request 3: Served by real-server-2     ← weight=2，出現較少
...
> **看懂結果**：10 次請求中，RS-1 出現 6 次 (60%)，RS-2 出現 4 次 (40%)，符合 WRR 3:2 權重！

#### Phase 2：負載分配精確度

```
▶ Phase 2: Load Distribution (50 requests)
──────────────────────────────
...
> **看懂結果**：50 次請求，RS-1 拿到 30 個 (60%)，RS-2 拿到 20 個 (40%)。**精確命中** WRR 3:2 = 60%:40% 的數學預期！

#### Phase 3：效能壓力測試

```
▶ Phase 3: Performance (500 req, 10 concurrent)
──────────────────────────────
Complete requests:      500
Failed requests:        0                   ← 零失敗！
Requests per second:    12965.12 [#/sec]    ← 每秒 ~13,000 請求
Time per request:       0.077 [ms]          ← 每次請求不到 0.1 毫秒
```

> **看懂結果**：500 個請求全部成功，零失敗。Layer-4 負載均衡的效能遠超 Layer-7 (Nginx proxy) 因為只做封包轉發，不解析 HTTP 內容。

---

### Step 5：模擬 Director 故障 (Failover)

這是 HA 高可用的核心測試 — 主 Director 掛了，備援 Director 能不能秒級接手？
...
```
🔄 Simulating Director Failover...

── Before failover ──
  VIP is currently on: lvs-director-1     ← VIP 目前在 MASTER

── Stopping Keepalived on Director-1 (MASTER) ──
  Waiting 5s for failover...

── After failover ──
  ✅ VIP moved to: lvs-director-2         ← VIP 自動轉移到 BACKUP！

── Testing VIP from client ──
{
  "server": "real-server-2",
  "ip": "192.168.100.22",
  "vip": "192.168.100.100",
  "timestamp": "2026-02-27T16:44:02Z"
}                                          ← Client 透過 VIP 仍能存取後端

✅ Failover test complete.
```

> **發生了什麼？**
> 1. Director-1 (MASTER, priority=100) 被停掉
> 2. Director-2 (BACKUP, priority=90) 偵測到 VRRP 心跳消失
> 3. Director-2 自動升級為 MASTER，接管 VIP (192.168.100.100)
> 4. Client 完全無感 — 同樣的 VIP 仍然能正常服務

---

### Step 6：恢復 Director (Recovery + Preemption)
...
```
🔧 Recovering Director-1...
  Waiting 5s for preemption...

── After recovery ──
  ✅ VIP is on: lvs-director-1            ← VIP 搶回到高優先權的 MASTER

✅ Recovery complete.
```

> **搶占 (Preemption)**：Director-1 恢復後，因為它的 priority (100) 高於 Director-2 (90)，VRRP 協定會自動把 VIP 搶回來。這就是 `preempt` 機制。

---

### Step 7：驗證 DSR (Direct Server Return)

DSR 的核心特徵是 **Director 只看到 Request，不看到 Response**。用 IPVS 統計數據來驗證：
...
> | 欄位 | 值 | 意義 |
> |------|-----|------|
> | `InPkts` | 140 | Director 收到 140 個進入封包 (Client 的 Request) |
> | **`OutPkts`** | **0** | **Director 轉發了 0 個出去的封包！** |
> | `InBytes` | 9020 | 進入的 Request 資料量 |
> | **`OutBytes`** | **0** | **Director 沒有轉發任何 Response 資料！** |
>
> **`OutPkts=0` 就是 DSR 的直接證據**：
> - Request 路徑：Client → Director (改 MAC) → Real Server
> - Response 路徑：Real Server → Client **直接回應，不經過 Director**
> - Director 完全沒碰到 Response 封包，所以 `OutPkts=0`

---

### Step 8：查看 Keepalived 日誌
...
> **看懂日誌**：
> - `Entering MASTER STATE` — Keepalived 決定自己是 MASTER
> - `Sending gratuitous ARP` — 廣播「VIP 192.168.100.100 在我這裡」讓所有人更新 ARP 快取
> - `Remote Web server ... succeed on service` — Health Check 確認兩台 RS 都健康
...
# 在 Director 內部：
root@lvs-director-1:/# ipvsadm -Ln              # 看 IPVS 路由表
root@lvs-director-1:/# ip addr show | grep .100  # 看 VIP 綁在哪
root@lvs-director-1:/# lvs-status.sh             # 完整狀態報告
...
# 在 Real Server 內部：
root@real-server-1:/# ip addr show lo            # 看 VIP 在 loopback 上
root@real-server-1:/# cat /proc/sys/net/ipv4/conf/all/arp_ignore   # 應為 1
root@real-server-1:/# cat /proc/sys/net/ipv4/conf/all/arp_announce  # 應為 2
root@real-server-1:/# curl http://localhost/health                   # 應回 OK
root@real-server-1:/# rs-status.sh               # 完整狀態報告
...
完成以上 10 個步驟，你已經實際操作過：

| 概念 | 對應步驟 | 你做了什麼 |
|------|---------|-----------|
| **IPVS (Linux Virtual Server)** | Step 3 | 看到 `ipvsadm -Ln` 的路由表 |
| **DSR (Direct Server Return)** | Step 7 | 用 `OutPkts=0` 證明 Response 不經過 Director |
| **WRR (Weighted Round Robin)** | Step 4 Phase 2 | 驗證 60%:40% 精確分配 |
| **VRRP Failover** | Step 5 | 停掉 MASTER，VIP 自動漂移到 BACKUP |
| **Preemption (搶占)** | Step 6 | MASTER 恢復後自動搶回 VIP |
| **Health Check** | Step 8 | Keepalived 日誌顯示 RS 健康檢查結果 |
| **ARP 抑制** | Step 9 | 確認 `arp_ignore=1`, `arp_announce=2` |
| **VIP on Loopback** | Step 9 | 確認 RS 的 lo 上有 VIP |
